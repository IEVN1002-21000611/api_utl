...
def registrar_alumno():
    try:
        ...
        if alumno!=None:
            ...
            cursor=con.connection.cursor()
            sql='''INSERT  INTO alumnos(matricula, nombre, apaterno, amaterno, con values('{0}','{1}','{2}','{3}','{4}',))'''.format(request.json['matricula'],request.json['nombre'],request.json['apaterno'],request.json['amaterno'],request.json['correo'], )
        cursor=con.connection.cursor()
